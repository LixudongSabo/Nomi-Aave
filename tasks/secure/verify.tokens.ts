...
import { eContractid, eContractidNomi, eNetwork, ICommonConfiguration, IReserveParams } from '../../helpers/types';
import { LendingPoolConfigurator__factory, LendingPool__factory } from '../../types/factories/nomi_protocol/lendingpool';

task('secure:tokens', 'Deploy oracles for dev enviroment')
  ...
  .setAction(async ({ verify, all, pool }, localDRE) => {
    ...
    for (const entry of Object.entries(getParamPerNetwork(ReserveAssets, network))) {
      ...
      await verifyContract(
        eContractidNomi.InitializableAdminUpgradeabilityProxy,
      ...
      if (aToken) {
        console.log('\n- Verifying aToken...\n');
        await verifyContract(eContractidNomi.AToken, await getAToken(aToken), [
      ...
      if (stableDebt) {
        ...
        await verifyContract(eContractidNomi.StableDebtToken, await getStableDebtToken(stableDebt), [
          lendingPoolProxy.address,
          tokenAddress,
